//! Scalar Lloyd-Max codebook construction.
//!
...
impl ScalarCodebook {
    pub fn new(bit_width: u8, dim: usize) -> Self {
        let levels = optimal_centroids(bit_width, dim);
...
fn optimal_centroids(bit_width: u8, dim: usize) -> Vec<f32> {
    ...
    match bit_width {
        ...
        2 => vec![-1.51, -0.453, 0.453, 1.51]
            ...
            .collect(),
        _ => lloyds_gaussian(1usize << bit_width, sigma),
    }
}
...
fn lloyds_gaussian(level_count: usize, sigma: f64) -> Vec<f32> {
    ...
    update_centroids(&mut centroids, &boundaries, sigma);
...
fn update_centroids(centroids: &mut [f64], boundaries: &[f64], sigma: f64) {
    ...
    centroids[centroids.len() - 1] =
        gaussian_conditional_expectation(sigma, boundaries[boundaries.len() - 1], f64::INFINITY);
}
