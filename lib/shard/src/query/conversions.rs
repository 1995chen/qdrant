...
use crate::query::payload_query::{PayloadQueryInternal, ResolvedTextQuery, TextQueryInternal};
    ...
    fn try_from_grpc_query(
        ...
        let scoring_query = match score {
            ...
            grpc::query_shard_points::query::Score::Payload(query) => {
                ScoringQuery::Payload(PayloadQueryInternal::try_from(query)?)
            }
...
impl From<ScoringQuery> for grpc::query_shard_points::Query {
    fn from(value: ScoringQuery) -> Self {
        ...
        match value {
            ScoringQuery::Payload(query) => Self {
...
impl From<PayloadQueryInternal> for grpc::RawPayloadQuery {
    fn from(value: PayloadQueryInternal) -> Self {
        match value {
            PayloadQueryInternal::Text(text) => Self::from(text),
        }
    }
}
...
impl TryFrom<grpc::RawPayloadQuery> for PayloadQueryInternal {
    ...
    fn try_from(value: grpc::RawPayloadQuery) -> Result<Self, Self::Error> {
        TextQueryInternal::try_from(value).map(Self::Text)
    }
}
...
#[cfg(test)]
mod payload_text_grpc_roundtrip_tests {
    use prost::Message;
    ...
    fn assert_payload_scoring_query_roundtrip(query: TextQueryInternal) {
        let original = ScoringQuery::Payload(PayloadQueryInternal::Text(query));
        let encoded = grpc::query_shard_points::Query::from(original.clone());
        assert!(matches!(
            &encoded.score,
            Some(grpc::query_shard_points::query::Score::Payload(_))
        ));
        let decoded = ScoringQuery::try_from_grpc_query(encoded, None).unwrap();
    ...
    fn unresolved_payload_text_query_roundtrip() {
        ...
        assert_payload_text_query_roundtrip(query.clone());
        assert_payload_scoring_query_roundtrip(query);
    }
