...
use self::query_enum::*;
use crate::query::payload_query::{
    PayloadQueryInternal, ResolvedTextQuery, TextQueryInternal, TextQueryStatsRequest,
};
...
impl ShardQueryRequest {
    ...
    fn visit_text_queries<'a>(
        ...
        if let Some(ScoringQuery::Payload(PayloadQueryInternal::Text(text))) = &self.query {
    ...
    fn visit_text_queries_mut<'a>(
        ...
        if let Some(ScoringQuery::Payload(PayloadQueryInternal::Text(text))) = &mut self.query {
            f(text, self.params.as_ref());
        }
...
impl ScoringQuery {
    pub fn target(&self) -> Option<QueryTarget<'_>> {
        match self {
            ...
            ScoringQuery::Payload(PayloadQueryInternal::Text(text)) => {
                Some(QueryTarget::PayloadField(&text.key))
            }
...
impl From<CoreSearchRequest> for ShardQueryRequest {
    fn from(value: CoreSearchRequest) -> Self {
        ...
        let query = match query {
            QueryEnum::Text(text) => ScoringQuery::Payload(PayloadQueryInternal::Text(text)),
            query @ (QueryEnum::Nearest(_)
            | QueryEnum::RecommendBestScore(_)
            | QueryEnum::RecommendSumScores(_)
            | QueryEnum::Discover(_)
            | QueryEnum::Context(_)
            | QueryEnum::FeedbackNaive(_)) => ScoringQuery::Vector(query),
        };
