...
use super::payload_query::{PayloadQueryInternal, TextQueryInternal};
...
impl From<PayloadQueryInternal> for QueryEnum {
    fn from(query: PayloadQueryInternal) -> Self {
        match query {
            PayloadQueryInternal::Text(text) => Self::Text(text),
        }
    }
}
