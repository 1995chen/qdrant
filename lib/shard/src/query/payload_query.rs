...
///
/// This wrapper intentionally stays separate from vector queries until query
/// planning lowers the concrete variant into a segment-level search request.
#[derive(Clone, Debug, Eq, PartialEq, Hash, Serialize)]
pub enum PayloadQueryInternal {
    Text(TextQueryInternal),
}

/// Shard-local statistics required to execute a payload text query.
///
...
pub struct TextQueryInternal {
    ...
    /// Shard-local query statistics resolved before segment search.
...
pub struct TextQueryStats {
    /// Whether at least one segment supplied statistics for the indexed field.
    pub index_available: bool,
    /// N for IDF, scoped by SearchParams.idf.corpus when present.
    pub doc_count: usize,
    pub tokens: Vec<String>,
    pub doc_frequencies: Vec<usize>,
    /// BM25 length-normalization corpus. This is always every visible indexed
    /// document in the shard and is independent of SearchParams.idf.corpus.
    pub global_doc_count: usize,
    pub global_sum_document_length: u64,
}

impl From<PayloadTextIndexStats> for TextQueryStats {
    fn from(stats: PayloadTextIndexStats) -> Self {
        Self {
            index_available: stats.index_available,
            doc_count: stats.document_count,
            tokens: stats.tokens,
            doc_frequencies: stats.document_frequencies,
            global_doc_count: stats.global_document_count,
            global_sum_document_length: stats.global_sum_document_length,
        }
    }
}
...
impl TextQueryStats {
    pub fn merge(&mut self, other: Self) -> OperationResult<()> {
        ...
        if self.doc_frequencies.len() != other.doc_frequencies.len() {
            return Err(OperationError::service_error(
                "text query statistics contain inconsistent token frequencies",
            ));
        }
        for (total, value) in self.doc_frequencies.iter_mut().zip(other.doc_frequencies) {
            *total = total.checked_add(value).ok_or_else(|| {
                OperationError::service_error("text query document frequency overflow")
            })?;
        }
        self.doc_count = self
            .doc_count
            .checked_add(other.doc_count)
            .ok_or_else(|| OperationError::service_error("text query document count overflow"))?;
        self.global_doc_count = self
            .global_doc_count
            .checked_add(other.global_doc_count)
            .ok_or_else(|| OperationError::service_error("BM25 document count overflow"))?;
        self.global_sum_document_length = self
            .global_sum_document_length
            .checked_add(other.global_sum_document_length)
            .ok_or_else(|| OperationError::service_error("BM25 document length sum overflow"))?;
        Ok(())
    }
    ...
    pub fn into_query_parts(self) -> (TextQueryTokenWeights, Option<OrderedFloat<f64>>) {
        let weights = self
            ...
            .collect();
        let average_document_length = (self.global_doc_count != 0).then(|| {
            OrderedFloat(self.global_sum_document_length as f64 / self.global_doc_count as f64)
        });
...
mod tests {
    ...
    #[test]
    fn idf_corpus_does_not_change_average_document_length() {
        let stats = TextQueryStats {
            index_available: true,
            // Empty explicit IDF corpus.
            doc_count: 0,
            tokens: vec!["alpha".to_string()],
            doc_frequencies: vec![0],
            // Non-empty global shard corpus.
            global_doc_count: 2,
            global_sum_document_length: 7,
        };

        let (weights, average_document_length) = stats.into_query_parts();
        assert_eq!(
            weights,
            vec![("alpha".to_string(), OrderedFloat(bm25_idf(0, 0)))]
        );
        assert_eq!(average_document_length, Some(OrderedFloat(3.5)));
    }

    #[test]
    fn merging_stats_rejects_empty_vs_nonempty_tokenization() {
        let mut empty = TextQueryStats {
            index_available: true,
            global_doc_count: 1,
            global_sum_document_length: 1,
            ..Default::default()
        };
        let nonempty = TextQueryStats {
            index_available: true,
            tokens: vec!["alpha".to_string()],
            doc_frequencies: vec![1],
            global_doc_count: 1,
            global_sum_document_length: 1,
            ..Default::default()
        };

        assert!(empty.merge(nonempty).is_err());
    }

    #[test]
    fn merging_stats_skips_segments_without_an_index() {
        let expected = TextQueryStats {
            index_available: true,
            doc_count: 1,
            tokens: vec!["alpha".to_string()],
            doc_frequencies: vec![1],
            global_doc_count: 1,
            global_sum_document_length: 2,
        };
        let mut merged = TextQueryStats::default();

        merged.merge(TextQueryStats::default()).unwrap();
        merged.merge(expected.clone()).unwrap();
        merged.merge(TextQueryStats::default()).unwrap();
