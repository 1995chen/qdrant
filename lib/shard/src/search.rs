...
pub fn search_load_profile(
    ...
    match query.capabilities().target {
        QueryTarget::Vector(vector_name) => {
            LoadProfile::for_search(vector_name, filter, with_payload)
        }
        QueryTarget::PayloadField(key) => {
            LoadProfile::for_payload_search(key, filter, with_payload)
        }
    }
}
...
impl CoreSearchRequest {
    ...
    pub fn load_profile(&self) -> LoadProfile {
        ...
        search_load_profile(query, filter.as_ref(), with_payload.as_ref())
    }

    /// Cost of the segment scoring leaf.
    ///
    /// Payload-text IDF statistics are resolved and rate-limited as a separate
    /// orchestration stage, so an `idf.corpus` filter is intentionally excluded here.
...
pub fn group_search_batches(searches: &[CoreSearchRequest]) -> Vec<QueryBatchGroup<'_>> {
    ...
    for search in searches {
        ...
        match groups.last_mut() {
            ...
            Some(_) | None => groups.push(QueryBatchGroup::from(SearchBatchGroup {
                params,
...
mod tests {
    ...
    use segment::types::{Condition, FieldCondition, HasIdCondition, IdfCorpusParams, IdfParams};
    ...
    #[test]
    fn text_query_rate_cost_excludes_separately_accounted_idf_corpus_filter() {
        let retrieval_filter = Filter::new_must(Condition::HasId(HasIdCondition::from(
        ...
        let expected = 1 + retrieval_filter.total_conditions_count();
        assert!(corpus.total_conditions_count() > 0);

        let mut request = text("rust", 3);
        request.filter = Some(retrieval_filter);
        request.params = Some(SearchParams {
            idf: Some(IdfParams::Corpus(IdfCorpusParams { corpus })),
            ..Default::default()
        });

        assert_eq!(request.query_rate_cost(), expected);
    }
