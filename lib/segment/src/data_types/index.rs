...
impl TextIndexBm25Config {
    pub const DEFAULT_K1: f64 = bm25::Bm25Params::DEFAULT_K1;
    pub const DEFAULT_B: f64 = bm25::Bm25Params::DEFAULT_B;
...
pub fn validate_bm25_b(b: f64) -> Result<(), ValidationError> {
    if bm25::validate_b(b).is_ok() {
        Ok(())
    } else {
        let mut error = ValidationError::new("range");
        error.message = Some(Cow::Borrowed("must be finite and in the range [0, 1]"));
        Err(error)
    }
}
