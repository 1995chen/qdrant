...
pub struct PayloadTextIndexStats {
    /// Whether the segment had a usable full-text index for this field.
    /// Missing or temporarily stale proxy indexes are skipped during shard aggregation.
    pub index_available: bool,
    pub tokens: Vec<String>,
    /// Document count in the requested IDF corpus.
    pub document_count: usize,
    /// Document-length sum in the requested corpus. Kept for proxy segments,
    /// which use their overlay filter to derive the effective global corpus.
    pub sum_document_length: u64,
    pub document_frequencies: Vec<usize>,
    /// All visible indexed documents, independent of the requested IDF corpus.
