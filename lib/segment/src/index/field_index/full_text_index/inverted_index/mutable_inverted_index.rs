...
use super::scoring::{
    Bm25SearchContext, DecodedDocumentLengths, IndexedBm25Posting, MutableBm25PostingListIter,
};
use super::{
    Bm25Params, Bm25State, Bm25Stats, Document, InvertedIndex, InvertedIndexScoring,
    MutableBm25State, ParsedQuery, TokenFrequencyMap, TokenId, TokenSet,
    bm25_scoring_not_enabled_error,
};
...
impl MutableInvertedIndex {
    ...
    pub(in crate::index::field_index::full_text_index) fn index_token_frequencies(
        ...
        // All backends score with the one-byte BM25 norm. Normalize while the
        // index is mutable too, so scores and aggregates do not change merely
        // because an optimizer converts this index to immutable/on-disk form.
        let document_length =
            EncodedDocumentLength::new(token_frequencies.document_length()).decoded();
...
impl InvertedIndex for MutableInvertedIndex {
    ...
    fn bm25_stats(&self) -> Option<Bm25Stats> {
        self.bm25.as_ref().map(|bm25| bm25.stats)
    }
...
impl InvertedIndexScoring for MutableInvertedIndex {
    fn search_text_index_plain(
        ...
        let Some(context) = self.scoring_context(query, params, top, is_stopped)? else {
            return Ok(Vec::new());
        };
        context.plain_search(
            &self.search_scratch_pool,
        ...
        F: Fn(PointOffsetType) -> bool,
    {
        ...
        context.search(&self.search_scratch_pool, filter)
    }
}
...
impl MutableInvertedIndex {
    fn scoring_context<'a>(
        ...
        Option<Bm25SearchContext<'a, MutableBm25PostingListIter<'a>, DecodedDocumentLengths<'a>>>,
    > {
        ...
        let mut resolved = query
            ...
            .filter_map(|query_token| {
                let token_id = *self.vocab.get(query_token.token())?;
                let posting = self.postings.get(token_id as usize)?;
                Some((token_id, posting.frequencies()?, query_token.idf()))
            })
            .collect::<Vec<_>>();
        resolved.sort_unstable_by_key(|(token_id, _, _)| *token_id);
        let postings = resolved
            .into_iter()
            .map(|(_, elements, idf)| {
                IndexedBm25Posting::new(MutableBm25PostingListIter::new(elements), idf)
            })
            .collect();
        Ok(Bm25SearchContext::new(
            postings,
            DecodedDocumentLengths(&bm25.document_lengths),
            params,
            bm25.stats,
