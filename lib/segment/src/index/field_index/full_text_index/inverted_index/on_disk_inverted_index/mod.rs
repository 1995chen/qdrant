...
use super::{
    Bm25Params, Bm25Stats, InvertedIndex, InvertedIndexScoring, ParsedQuery,
...
impl<S: UniversalRead> OnDiskInvertedIndex<S> {
    ...
    pub fn open(
        ...
        let mut bm25 = if has_frequencies {
        ...
        let mut persisted_empty = BitVec::repeat(false, total_count);
        ...
        // Persisted BM25 aggregates already exclude the build-time empty mask.
        // Reconcile only documents deleted by the authoritative ID tracker;
        // subtracting the union would double-apply a deletion already reflected
        // by the immutable index and would read one norm for every empty slot.
        if let Some(bm25) = &mut bm25 {
            let deletions = deleted
                .iter_ones()
                .filter(|&point_id| !persisted_empty[point_id])
                .map(|point_id| (point_id, ReadRange::one(point_id as u64)));
            let super::Bm25State {
                document_lengths,
                stats,
            } = bm25;
            document_lengths.read_batch(deletions, Random, |point_id, encoded: &[u8]| {
                let Some(&encoded_length) = encoded.first() else {
                    return Err(OperationError::service_error(format!(
                        "BM25 document length is missing for deleted point {point_id}",
                    )));
                };
                let document_length = EncodedDocumentLength::from_encoded(encoded_length).decoded();
                stats.remove_document(document_length);
                Ok(())
            })?;
        }

        deleted.bitor_assign(persisted_empty.as_bitslice());
...
impl<S: UniversalRead> InvertedIndex for OnDiskInvertedIndex<S> {
    ...
    fn bm25_stats(&self) -> Option<Bm25Stats> {
        self.storage.bm25.as_ref().map(|bm25| bm25.stats)
    }
