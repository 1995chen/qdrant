...
use super::{
    Bm25Params, Bm25Stats, Document, InvertedIndex, InvertedIndexScoring, ParsedQuery,
...
impl InvertedIndex for ImmutableInvertedIndex {
    ...
    fn remove(&mut self, idx: PointOffsetType) -> bool {
        ...
        if let Some(bm25) = &mut self.bm25
            && let Some(&encoded_length) = bm25.document_lengths.get(idx as usize)
        {
    ...
    fn try_from(index: MutableInvertedIndex) -> OperationResult<Self> {
        ...
        let bm25 = bm25.map(|bm25| {
            let document_lengths = bm25
                ...
                .collect::<Vec<_>>();
            // Immutable scorers consume decoded one-byte norms. Define their
            // aggregate over those same decoded lengths so later deletions can
            // be subtracted exactly without retaining another per-document
            // length representation.
            let mut stats = Bm25Stats::default();
            for &encoded_length in &document_lengths {
                stats.add_document(EncodedDocumentLength::from_encoded(encoded_length).decoded());
            }
            super::Bm25State {
                document_lengths,
                stats,
