...
/// BM25 state with decoded one-byte norm lengths used by mutable indexes.
/// Keeping them as `u32` avoids repeated decoding while preserving the same
/// normalization semantics as immutable/on-disk indexes.
...
pub trait InvertedIndex {
    ...
    fn bm25_stats(&self) -> Option<Bm25Stats>;
...
mod tests {
    ...
    use super::{
        ARRAY_BOUNDARY_SENTINEL, Bm25Params, Bm25Stats, Document, InvertedIndex,
        InvertedIndexScoring, ParsedQuery, TokenFrequencyMap, TokenId, TokenSet,
    };
    ...
    use crate::types::{QueryTokenWeight, QueryTokenWeightSet};
    ...
    #[test]
    fn test_immutable_bm25_state_follows_deletions() {
        let mut index =
            ImmutableInvertedIndex::try_from(bm25_mutable_inverted_index(false)).unwrap();
        assert!(index.remove(0));
        ...
        assert_eq!(bm25.document_lengths, [4, 2, 0]);
    }

    #[test]
    fn test_on_disk_bm25_state_applies_deletions_on_open() {
        let immutable =
            ImmutableInvertedIndex::try_from(bm25_mutable_inverted_index(false)).unwrap();
    ...
    #[test]
    fn test_on_disk_bm25_state_does_not_reapply_persisted_empty_mask() {
        let mut immutable =
            ImmutableInvertedIndex::try_from(bm25_mutable_inverted_index(false)).unwrap();
        assert!(immutable.remove(0));
        let expected = immutable.bm25_stats();
        ...
        let mmap = OnDiskInvertedIndex::open(
            ...
            false,
            true,
            BitVec::new().as_bitslice(),
        )
        .unwrap()
        .unwrap();

        assert_eq!(mmap.bm25_stats(), expected);
    }

    #[test]
    fn test_bm25_score_is_stable_across_length_norm_quantization() {
        let mut mutable = MutableInvertedIndex::new(false, true);
        let token_ids = mutable.register_tokens(std::iter::repeat_n("alpha", 41));
        mutable
            .index_token_frequencies(
                0,
                TokenFrequencyMap::from_tokens(&token_ids, None),
        ...
        // 41 is the first length that the one-byte norm rounds down.
        let mutable_stats = mutable.bm25_stats().unwrap();
        assert_eq!(mutable_stats.sum_doc_len, 40);
        assert_eq!(mutable.bm25.as_ref().unwrap().document_lengths, [40]);

        let immutable = ImmutableInvertedIndex::try_from(mutable.clone()).unwrap();
        assert_eq!(immutable.bm25_stats(), Some(mutable_stats));

        let query = QueryTokenWeightSet::new(vec![QueryTokenWeight::new("alpha".into(), 1.0)]);
        let params = Bm25Params { k1: 1.2, b: 0.75 };
        let is_stopped = AtomicBool::new(false);
        let mutable_score = mutable
            .search_text_index(&query, params, 1, &is_stopped, |_| true)
            .unwrap()[0]
            .score;
        let immutable_score = immutable
            .search_text_index(&query, params, 1, &is_stopped, |_| true)
            .unwrap()[0]
            .score;
        ...
        .unwrap()
        .unwrap();
        assert_eq!(mmap.bm25_stats(), Some(mutable_stats));
        let mmap_score = mmap
            .search_text_index(&query, params, 1, &is_stopped, |_| true)
            .unwrap()[0]
            .score;

        assert_eq!(mutable_score, immutable_score);
        assert_eq!(mutable_score, mmap_score);
    }
