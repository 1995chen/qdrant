...
impl<S: UniversalRead> FullTextIndexRead for ReadOnlyFullTextIndex<S> {
    ...
    fn bm25_document_stats(&self) -> Option<(usize, u64)> {
        match self {
            ReadOnlyFullTextIndex::Appendable(index) => index.bm25_document_stats(),
            ReadOnlyFullTextIndex::OnDisk(index) => index.bm25_document_stats(),
            ReadOnlyFullTextIndex::Immutable(index) => index.bm25_document_stats(),
        }
    }
