...
def expected_tf(count, document_length, average_document_length):
    # The test index uses k1=1 and b=0.75.
    normalization = 1.0 * (
        (1.0 - 0.75) + 0.75 * document_length / average_document_length
    )
    return count * 2.0 / (count + normalization)
...
def setup(collection_name):
    ...
    response = request_with_validation(
        ...
        body={
            # Payload BM25 statistics are intentionally shard-local in the
            # initial design, so score assertions use one shard.
            "shard_number": 1,
            "vectors": {
                "size": 1,
                "distance": "Dot",
...
def test_payload_text_query_scores_and_updates_live_idf(collection_name):
    ...
    average_document_length = 6.0 / 4.0

    assert [point["id"] for point in results] == [0, 1]
    assert results[0]["score"] == pytest.approx(
        expected_tf(2, 2, average_document_length) * idf
    )
    assert results[1]["score"] == pytest.approx(
        expected_tf(1, 2, average_document_length) * idf
    )
    ...
    live_average_document_length = 4.0 / 3.0

    assert [point["id"] for point in results] == [1]
    assert results[0]["score"] == pytest.approx(
        expected_tf(1, 2, live_average_document_length) * live_idf
    )
...
def test_payload_text_query_uses_corpus_scoped_idf(collection_name):
    ...
    # The corpus only changes N/df. Retrieval and BM25 length normalization
    # still use their normal scopes; the global average is 6 / 4.
    assert [point["id"] for point in results] == [0, 1]
    assert results[0]["score"] == pytest.approx(expected_tf(2, 2, 1.5) * corpus_idf)
    assert results[1]["score"] == pytest.approx(expected_tf(1, 2, 1.5) * corpus_idf)
    ...
    assert [point["id"] for point in results] == [0, 1]
    # An empty IDF corpus still uses the shard-global average length. In
    # particular, it must not fall back to each segment's local average when
    # length normalization (b != 0) is enabled.
    assert results[0]["score"] == pytest.approx(
        expected_tf(2, 2, 1.5) * empty_corpus_idf
    )
    assert results[1]["score"] == pytest.approx(
        expected_tf(1, 2, 1.5) * empty_corpus_idf
    )
